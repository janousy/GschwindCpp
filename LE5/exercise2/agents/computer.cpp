...
template <typename F>
vector<int> computer<F>::possibleStonePosition(const F &pf) {
    vector<int> pos;
...
template <typename F>
int computer<F>::sum_direction(char pS, int hOrg, int wOrg, int hDir, int wDir, const F &pf) {
    int h = hOrg + hDir;
    int w = wOrg + wDir;
    int sum = 0;
...
template <typename F>
int computer<F>::canWin(const F &pf) {
...
//-1 if no possible pos to win the game
template <typename F>
int computer<F>::otherPlayerCanWin(const F &pf){
...
template <typename F>
int computer<F>::maximizeConnectedStones(const F &pf) {
